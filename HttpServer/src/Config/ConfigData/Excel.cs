...
public partial class Excel_Data
{
	public int Id;
	public bool Open;
	public float Time;
...
public partial class Excel_Table : BaseTable
{
    public override string GetTableName() { return "Excel"; }
    private List<Excel_Data> DataList;
    public int Count {  get { return DataList.Count; } }
    public Excel_Data GetItem(int index)
    {
    ...
    {
        DataList = new List<Excel_Data>();
        var list = Newtonsoft.Json.JsonConvert.DeserializeObject<Excel_Data[]>(json);
        var len = list.Length;
        for (int i = 0; i < len; i++)
        {
            DataList.Add(list[i]);
