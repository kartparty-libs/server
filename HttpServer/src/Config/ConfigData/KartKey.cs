using System;
using System.Collections.Generic;
using Newtonsoft.Json;
public partial class KartKey_Data
{
	public string Id;
	public int Pledge;
	public int Diamond;
	public int Gold;
	public float PledgeRate;
	public int IsRobot;
	public int RobotRank;
	public string RobotName;
	public string RobotEmail;
	public int RobotMiningType;

}
public partial class KartKey_Table : BaseTable
{
    public override string GetTableName() { return "KartKey"; }
    private List<KartKey_Data> DataList;
    public int Count {  get { return DataList.Count; } }
    public KartKey_Data GetItem(int index)
    {
        if (index >= DataList.Count || index < 0) return null;
        return DataList[index];
    }
    public override void LoadJson(string json)
    {
        DataList = new List<KartKey_Data>();
        var list = Newtonsoft.Json.JsonConvert.DeserializeObject<KartKey_Data[]>(json);
        var len = list.Length;
        for (int i = 0; i < len; i++)
        {
            DataList.Add(list[i]);
        }
#if true
            PostLoadJson();
#endif
    }
    
#if true

    private Dictionary<string, int> _keyToIndex;
    private void PostLoadJson()
    {
        int len = DataList.Count;
		_keyToIndex = new Dictionary<string, int>(len);
		for (int i = 0;i < len; i++)
		{
			var item = DataList[i];
			var k = item.Id;
			if (!_keyToIndex.ContainsKey(k))
			{
				_keyToIndex.Add(k, i);
			}
			else
			{

			}
		}
    }
    public KartKey_Data Get(string key)
    {
        if (_keyToIndex != null && _keyToIndex.TryGetValue(key, out int index))
        {
            return GetItem(index);
        }
        return null;
    }
#endif

}
