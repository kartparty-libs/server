...
public partial class Rank_Data
{
	public int Id;
	public string[] RankSnapshotTime; 

}
public partial class Rank_Table : BaseTable
{
    public override string GetTableName() { return "Rank"; }
    private List<Rank_Data> DataList;
    public int Count {  get { return DataList.Count; } }
    public Rank_Data GetItem(int index)
    {
    ...
    {
        DataList = new List<Rank_Data>();
        var list = Newtonsoft.Json.JsonConvert.DeserializeObject<Rank_Data[]>(json);
        var len = list.Length;
        for (int i = 0; i < len; i++)
        {
            DataList.Add(list[i]);
