...
public partial class Gift_Data
{
	public int Id;
	public int Source;
	public int[] ItemIds; 
...
public partial class Gift_Table : BaseTable
{
    public override string GetTableName() { return "Gift"; }
    private List<Gift_Data> DataList;
    public int Count {  get { return DataList.Count; } }
    public Gift_Data GetItem(int index)
    {
    ...
    {
        DataList = new List<Gift_Data>();
        var list = Newtonsoft.Json.JsonConvert.DeserializeObject<Gift_Data[]>(json);
        var len = list.Length;
        for (int i = 0; i < len; i++)
        {
            DataList.Add(list[i]);
