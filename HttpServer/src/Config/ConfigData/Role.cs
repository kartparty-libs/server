...
public partial class Role_Data
{
	public int Id;

}
public partial class Role_Table : BaseTable
{
    public override string GetTableName() { return "Role"; }
    private List<Role_Data> DataList;
    public int Count {  get { return DataList.Count; } }
    public Role_Data GetItem(int index)
    {
    ...
    {
        DataList = new List<Role_Data>();
        var list = Newtonsoft.Json.JsonConvert.DeserializeObject<Role_Data[]>(json);
        var len = list.Length;
        for (int i = 0; i < len; i++)
        {
            DataList.Add(list[i]);
