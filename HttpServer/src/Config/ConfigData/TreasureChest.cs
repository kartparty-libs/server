...
public partial class TreasureChest_Data
{
	public int Id;
	public int Type;
	public int[] RandomScore; 
	public int[] FusionParam; 

}
public partial class TreasureChest_Table : BaseTable
{
    public override string GetTableName() { return "TreasureChest"; }
    private List<TreasureChest_Data> DataList;
    public int Count {  get { return DataList.Count; } }
    public TreasureChest_Data GetItem(int index)
    {
    ...
    {
        DataList = new List<TreasureChest_Data>();
        var list = Newtonsoft.Json.JsonConvert.DeserializeObject<TreasureChest_Data[]>(json);
        var len = list.Length;
        for (int i = 0; i < len; i++)
        {
            DataList.Add(list[i]);
