...
public partial class Head_Data
{
	public int Id;

}
public partial class Head_Table : BaseTable
{
    public override string GetTableName() { return "Head"; }
    private List<Head_Data> DataList;
    public int Count {  get { return DataList.Count; } }
    public Head_Data GetItem(int index)
    {
    ...
    {
        DataList = new List<Head_Data>();
        var list = Newtonsoft.Json.JsonConvert.DeserializeObject<Head_Data[]>(json);
        var len = list.Length;
        for (int i = 0; i < len; i++)
        {
            DataList.Add(list[i]);
