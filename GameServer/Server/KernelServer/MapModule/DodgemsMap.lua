...
local CDodgemsMap = ClassRequire("CDodgemsMap")
...
--场景初始化
function CDodgemsMap:_constructor(i_nCfgID, i_nInstID, i_oManager, i_tAppendInfo, i_oObserver, i_bOccupy)
    CDodgemsMap._super._constructor(self, i_nCfgID, i_nInstID, i_oManager, i_tAppendInfo, i_oObserver, i_bOccupy)
end

function CDodgemsMap:Init()
    -- 游戏状态
    self.m_eGameState = 0
    -- 加载地图玩家信息
...
--场景刷新
function CDodgemsMap:Update(i_nDeltaMsec)
    CDodgemsMap._super.Update(self, i_nDeltaMsec)
...
-- 玩家进入
function CDodgemsMap:PlayerEnter(i_oPlayer)
    CDodgemsMap._super.PlayerEnter(self, i_oPlayer)
end

-- 踢出地图
function CDodgemsMap:LeaveMap()
...
-- 玩家离开
function CDodgemsMap:OnPlayerLeave(i_oPlayer)
    CDodgemsMap._super.OnPlayerLeave(self, i_oPlayer)
...
-- 准备游戏
function CDodgemsMap:ReadyGame()
    if self.m_eGameState == DodgemsMapState.ReadyGame then
        return
    end
    self.m_eGameState = DodgemsMapState.ReadyGame

    self:SendToAllPlayer("C_CompetitionGameState", self.m_eGameState)
end

-- 开始游戏
function CDodgemsMap:StartGame()
    if self.m_eGameState == DodgemsMapState.StartGame then
        return
    end
    self.m_eGameState = DodgemsMapState.StartGame
    self.m_nLoadMapCompleteTime = 0
...
-- 结束游戏
function CDodgemsMap:EndGame()
    if self.m_eGameState == DodgemsMapState.EndGame then
        return
    end
    self.m_eGameState = DodgemsMapState.EndGame
...
-- 加载地图完成
function CDodgemsMap:SyncPlayerLoadMapComplete(i_oPlayer)
...
-- 同步玩家状态
function CDodgemsMap:SyncPlayerStateInfo(i_oPlayer, i_tState)
...
-- 同步完成比赛
function CDodgemsMap:PlayerCompleteGame(i_oPlayer, i_nTime, i_sRobotRoleId, i_sRobotName)
...
function CDodgemsMap:PlayerAddBuff(i_oPlayer, i_sAddBuffRoleId, i_nBuffType, i_tBuffData)
    local bPlayer = self:GetPlayer(i_sAddBuffRoleId)
    if bPlayer then
        bPlayer:SendToClient("C_PlayerAddBuff", i_nBuffType, i_tBuffData)
    end
end
...
-- 玩家添加buff
defineC.K_PlayerAddBuffReq = function(i_oPlayer, i_sAddBuffRoleId, i_nBuffType, i_tBuffData)
    if i_oPlayer:GetState() == KSPlayerStateEnum.eInGame and i_oPlayer:GetMap() and not i_oPlayer:IsInCommonMap() then
        i_oPlayer:GetMap():PlayerAddBuff(i_oPlayer, i_sAddBuffRoleId, i_nBuffType, i_tBuffData)
    end
end
